...
<style>
    ...
    .explorer-header h1 { color: white; margin: 0; font-size: 2rem; }
    .explorer-header p { color: #aaa; margin: 0.3rem 0 0; }
...
<div class="explorer-header">
    <h1>📋 IOC Explorer</h1>
    <p>Search, filter and analyze all collected Indicators of Compromise</p>
</div>
""", unsafe_allow_html=True)
...
st.markdown(f"""
<div style="background:#eaf4fb; border-radius:10px; padding:0.8rem 1.2rem; margin-bottom:1rem;
            border-left:5px solid #2980b9; font-size:1rem">
    Showing <b>{f_total:,}</b> of <b>{total:,}</b> total IOCs
    {' — <b style="color:#e74c3c">Filter active</b>' if f_total < total else ''}
</div>
""", unsafe_allow_html=True)
...
with col_left:
    ...
    fig1 = px.bar(tactic_df, x="Count", y="Tactic", orientation="h",
                  ...
                  title="IOCs by MITRE Tactic", height=300, text="Count")
...
with col_right:
    ...
    fig2 = px.bar(country_df, x="Country", y="Count",
                  color="Count", color_continuous_scale=["#fff5f5","#e74c3c"],
                  title="Top Countries", height=300, text="Count")
...
with col_dl1:
    ...
    st.download_button("⬇️ Export as CSV", csv, "iocs_export.csv", "text/csv",
                       use_container_width=True)
with col_dl2:
    json_data = filtered.to_json(orient="records", indent=2)
    st.download_button("⬇️ Export as JSON", json_data, "iocs_export.json", "application/json",
                       use_container_width=True)